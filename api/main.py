...
def get_entities(db: Session = Depends(get_db)):
    ...
    try:
        ...
        response["err"] = str(e)
        raise HTTPException(status_code=500, detail=JSONResponse(content=jsonable_encoder(response)))
...
def get_lastseen(db: Session = Depends(get_db)):
    ...
    try:
        response["data"] = [{"ip": ip, "mac": mac} for mac, ip in fetch_lastseen(db).all()]
    except BaseException as e:
        response["err"] = str(e)
        raise HTTPException(status_code=500, detail=JSONResponse(content=jsonable_encoder(response)))
